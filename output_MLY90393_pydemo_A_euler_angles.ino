...
//  float pitch = -atan2(x1, y1) * RAD_TO_DEnäG;
  float pitch = atan2(x1, sqrt(y1 * y1 + z1 * z1)) * RAD_TO_DEG;
  float roll = atan2(y1, z1) * RAD_TO_DEG;
  // Assuming we do not have any tilt (roll, pitch are ignored)
  float yaw = atan2(y1, x1) * RAD_TO_DEG;
//float yaw =atan2(-x1, sqrt(y1 * y1 + z1 * z1)) * RAD_TO_DEG;
  
  // Output the Euler angles

  
//  Serial.print(" X: "); Serial.print(pitch);
//  Serial.print(" Y: "); Serial.print(roll);
